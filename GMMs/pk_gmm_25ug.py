...
def sm_ggm(inp,metric="aic"):
    ...
    for i1 in np.arange(10):
        ...
        if i1 in (0,7,8,9):
            Y_v_list_2.reverse()
        Y_i = np.argsort(Y_v_list_2)
        Y_i = list(Y_i)
        for index_v in enumerate(Y_):
            gmm_10_scores[index_v[0]+1].append(Y_i.index(index_v[1]))
        ...
        plt.tight_layout()
        #plt.savefig("pk_gmm_{0}_25ug".format(i1),dpi=300)
...
sm_ggm(np.abs(np.reshape(np.array(pk_a_25),(-1,10))))
...
#gmm_10.to_csv("gmm_10_AIC_25ug.csv",index=False,header=False)

gmm_10_scores = pd.DataFrame(gmm_10_scores)
#gmm_10_scores.to_csv("gmm_10_scores_25ug.csv",index=False,header=False)
########
...
def sm_ggm_10(inp):
    ...
    for i1 in np.arange(1):
        ...
        plt.subplots_adjust(hspace=.35, bottom=.02)
        plt.tight_layout()
        plt.savefig("Plots/PK/pk_gmm_full_p25ug.pdf",dpi=300)
        plt.show()
        return(Y_)
        break
...
c_LLL = sm_ggm_10(np.abs(np.reshape(np.array(pk_a_25),(-1,10))))

gmm_all = pd.DataFrame(gmm_all)
gmm_all.to_csv("Data/PK/gmm_pk_full_aic_p25ug.csv",index=False,header=False)

gmm_all_scores = pd.DataFrame(gmm_all_scores)
gmm_all_scores.to_csv("Data/PK/gmm_pk_full_scores_p25ug.csv",index=False,header=False)
